...
public class Bar {
	...
	public Bar(int number, Point center, int id) {
		...
		if (column % 2 == 0) {
			...
			halfRow = column / 2;
			startLeft = new Point(center.x - 0.5 * Player.HORIZONTAL_GAP,
					center.y - halfRow * Player.VERTICAL_GAP + 0.5 * Player.VERTICAL_GAP);
			startRight = new Point(center.x + 0.5 * Player.HORIZONTAL_GAP,
					center.y - halfRow * Player.VERTICAL_GAP + 0.5 * Player.VERTICAL_GAP);

		} else {
			System.out.println("Odd number in a column");
			halfRow = (column - 1) / 2;
			startLeft = new Point(center.x - 0.5 * Player.HORIZONTAL_GAP, center.y - halfRow * Player.VERTICAL_GAP);
			startRight = new Point(center.x + 0.5 * Player.HORIZONTAL_GAP, center.y - halfRow * Player.VERTICAL_GAP);
		}
		...
		topLeft = startLeft;
		topRight = startRight;
		bottomLeft = new Point(startLeft.x, startLeft.y + (column - 1) * Player.VERTICAL_GAP);
		bottomRight = new Point(startRight.x, startRight.y + (column - 1) * Player.VERTICAL_GAP);
		...
		for (int i = 0; i < column; i++) {
			Point leftPlayer = new Point(startLeft.x, startLeft.y + i * Player.VERTICAL_GAP);
			Point rightPlayer = new Point(startLeft.x + Player.HORIZONTAL_GAP, startLeft.y + i * Player.VERTICAL_GAP);
	...
	public Point goUp(Point me) {
		Point newLoc = new Point(0, -Player.VERTICAL_GAP);
		return newLoc;
	}
	...
	public Point goRight(Point me) {
		Point newLoc = new Point(Player.HORIZONTAL_GAP, 0);
		return newLoc;
	}
	...
	public Point goLeftToNextBar(Point me) {
		...
		// find previous bar
		Bar prev = Player.bars.get(this.barId - 1);
	...
	public Point goRightToNextBar(Point me) {
		...
		// find previous bar
		Bar next = Player.bars.get(this.barId + 1);
	...
	public Point move(Point dancer) {
		...
		if (dancer.x < center.x) {
			...
			if (ToolBox.compareDoubles(dancer.y, topLeft.y)) {
				...
				newLoc = new Point(Player.HORIZONTAL_GAP, 0);
			}
			// else, just go up
			else {
				// newLoc = new Point(dancer.x, dancer.y - VERTICAL_GAP);
				newLoc = new Point(0, -Player.VERTICAL_GAP);
			}
		}
		// right column
		else {
		...
		if (!inPlace(left) && !inPlace(right)) { // redundant but ok
			...
			for (int i = 0; i < dancers.length; i++) {
				...
				if (dancers[i].x < center.x && dancers[i].y > left.y
						&& ToolBox.compareDoubles(dancers[i].y, left.y + Player.VERTICAL_GAP)) {
					first = i;
					...
					// bottom
					if (ToolBox.compareDoubles(left.y + Player.VERTICAL_GAP, bottomRight.y) && dancers[i].x > center.x
							&& ToolBox.compareDoubles(dancers[i].y, left.y + Player.VERTICAL_GAP)) // bottom
																									// right
						second = i;
					// otherwise it's just 2 down, so makes sure it's not right
					// about the stack, it's on the left, and 2 down
					else if (!ToolBox.compareDoubles(left.y + Player.VERTICAL_GAP, bottomRight.y)
							&& dancers[i].x < center.x
							&& ToolBox.compareDoubles(dancers[i].y, left.y + 2 * Player.VERTICAL_GAP))
						second = i;
				}
			}
			...
			if (first >= 0 && second >= 0) {
				...
				moves.put(leftNum, new Point(0, Player.VERTICAL_GAP)); // left
																		// moves
																		// down
				System.out.println();
			} else {
	...
	public boolean inPlace(Point dancer) {
		...
		if (ToolBox.compareDoubles(dancer.y, bottomRight.y)) { // just got into
																...
																// out of
																// rotation
			bottomRight = bottomRight.add(new Point(0, -Player.VERTICAL_GAP));
			return true;
		}
		return false;
	}
	...
	public Point innerSwap(Point dancer) {
		...
		int relativeId = ToolBox.findRelativeIdInBar(dancer, this);
		...
		if (dancer.x < center.x) {
			...
			if (dancer.y + Player.VERTICAL_GAP > bottomLeft.y)
				return new Point(0.0, 0.0);
			else {
				if (relativeId % 4 == 2)
					return new Point(0, Player.VERTICAL_GAP);
				if (relativeId % 4 == 0)
					return new Point(0, -Player.VERTICAL_GAP);
			}
		...
		// right column
			...
			if (ToolBox.compareDoubles(dancer.y + Player.VERTICAL_GAP, bottomRight.y))
				return new Point(0, 0);
			else {
				if (relativeId % 4 == 1)
					return new Point(0, Player.VERTICAL_GAP);
