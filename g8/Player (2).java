...
public class Player implements sqdance.sim.Player {
    ...
    public Point[] test(){
        ...
        int x = (int)((room_side - 2) / PAIR_DIST);

        Point curr = new Point(0,0);

        if(x>=pairs){
            for(int i=0; i<pairs;i++){
                locs[i] = new Point(PARTNER_DIST*(i+1),PAIR_DIST);
            }

            for(int i=total_dancers-1; i>=pairs;i--){
                locs[i] = new Point(PARTNER_DIST*(i-pairs+1),PAIR_DIST*2);
            }
        }else{
            int j = 1;
            int g = 4;
...
if(f){
                    locs[i] = new Point(PARTNER_DIST*(i+1),PAIR_DIST*z);
                }else{
                    locs[i] = new Point(PARTNER_DIST*(22-j+1),PAIR_DIST*g);
                    j++;
                }
                if((i+1)%22 == 0){
                    f=!f;
